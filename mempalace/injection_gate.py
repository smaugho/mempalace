...
def run_state_judge(
    *,
    ...
    system_prompt = (
        ...
        "Be assertive. If the transcript shows that an entity's "
        "stored current_state is plausibly out of date -- a todo "
        "advanced, an agent's focus shifted, a Task moved phases, a "
        "field that should reflect new activity does not -- FLAG IT. "
        "A false positive costs the agent one state_deltas patch; a "
        "false negative leaves stale state in the graph. When the "
        "transcript and the current_state diverge, you flag.\n\n"
        "Always emit via the report_state_changes tool. An empty "
        "changes array means 'no state changes detected' -- only "
        "appropriate when the followed entities genuinely have not "
