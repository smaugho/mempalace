...
NEVER_STOP_BLOCK_REASON = (
    ...
    "BANNED-PHRASE SELF-CHECK (from CLAUDE.md). Before stopping, scan your "
    "draft for any of these. If you generated ANY of them, the sentence is "
    "a LIE -- delete it, open TaskList, pick the next non-completed task, "
    "declare a work intent, start working:\n"
    "  - 'carry-forward' / 'carry forward' / 'deferred to...' / 'defer to...'\n"
    "  - 'next session' / 'another session' / 'future session' / 'later session'\n"
    "  - 'session scope' / 'this session\\'s scope' / 'not in scope for this session'\n"
    "  - 'multi-day arc' / 'multi-week' / 'warrants its own session' / "
    "'warrants its own dedicated session' / 'dedicated session'\n"
    "  - 'honest deferral' / 'realistic scope' / 'achievable in this turn'\n"
    "  - 'I\\'ll pause here' / 'wrap up' / 'wrapping up' (with pending tasks)\n"
    "  - 'should I continue?' / 'want me to keep going?' / 'anything else?'\n"
    "  - 'context window doesn\\'t allow' / 'remaining context' / 'context budget'\n"
    "  - 'this is a substantial arc' / 'this is genuinely multi-day'\n\n"
    "THE ONLY THREE LEGITIMATE STOPS:\n"
    "  (a) ALL WORK DONE: TaskList shows ZERO [pending] / [in_progress] "
    "items AND every completed task has a verified proof artifact in the "
    "final report. Proof shape varies by project: PR link with green CI "
    "(if repo has CI) or commit hash for code; project-appropriate "
    "verification evidence for UI/visible-output changes (browser "
    "screenshot, E2E test pass, manual click-through note -- the TOOL "
    "varies, the requirement 'verified end-to-end' does not); integration "
    "test or live curl/parse for backend; grep/diff + runtime check for "
    "data/config. The point is a verifiable artifact someone else could "
    "re-check, not the brand of tool.\n"
    "  (b) REAL HARD BLOCKER requiring user input -- use AskUserQuestion. "
    "Real means: credential only user holds, irreversible destructive "
    "action lacking prior consent, or requirement so ambiguous all paths "
    "blocked AFTER you tried to resolve it yourself. NOT real blockers: "
    "'multi-day arc' (start the first step), 'tool not permitted by "
    "intent' (declare the right intent), 'low tool budget' (call "
    "mempalace_extend_intent), 'diary write failed' (fix the root error), "
    "'should I ask the user' (no -- they said work till done).\n"
    "  (c) USER EXPLICITLY TYPED 'stop' IN CHAT (not this hook -- the "
    "actual user, in a new chat message).\n\n"
    "If (a/b/c) genuinely applies and you ARE stopping cleanly:\n"
    ...
    "slots={'subject': ['<session_topic_entity>']}, ...)`.\n"
    ...
    "patterns ('pending', 'TODO', 'unresolved', 'not yet', 'next step') "
    ...
    "content='<narrative with verified proof artifacts per task>')`.\n\n"
    "If you actually have more work, DO IT instead of trying to stop. "
    "Adrian directives 2026-05-09, 2026-05-24, 2026-05-28."
)
