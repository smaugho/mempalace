...
- mempalace_declare_intent(intent_type, slots, context, agent, budget, cause_id, initial_intent_state) -- Declare what you intend to do; returns permissions + injected memories (P4.4). `cause_id` (mandatory) links upward to the user-intent context or a Task entity -- there is no parentless action ('autonomous' was removed 2026-05-30). `initial_intent_state` is validated against state_schemas.intent_state.
