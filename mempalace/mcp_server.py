            ...
            "properties": {
                ...
                "is_a": {
                    "description": (
                        ...
                        "is_a edge(s) are written atomically with the "
                        "entity insert -- no second kg_add call "
                        "needed. Each target must be a declared "
                        "kind='class' entity. Without is_a, instance "
                        "entities float ungrounded with no class "
                        "membership -- breaking predicate cardinality "
                        "checks, retrieval class filters, and the "
                        "gardener's link-author channel. NOT required "
                        "for kind='record' (records describe entities, "
                        "they don't is_a anything), kind='class' "
                        "(subclass declarations link to their parent "
                        "via a separate kg_add(is_a, parent) call), "
                        "kind='predicate' (predicates carry their own "
                        "constraints), or kind='literal' (literals are "
                        "values, not entities). When ANY is_a target "
                        "carries state_updatable=True AND kind='entity' "
                        "(this entity is an instance, not a sub-class), "
                        "initial_state becomes REQUIRED too -- it is "
                        "validated against the target's state_schema "
            ...
            "properties": {
                ...
                "cause_id": {
                    ...
                    "description": (
                        ...
                        "Parent-cause id for this activity-intent. There "
                        "is NO parentless action -- every intent must be "
                        "caused by a user instruction or a Task. EXACTLY "
                        "TWO accepted forms:\n"
                        ...
                        "Task) for background / scheduled / "
                        "self-initiated work: declare a Task that states "
                        "what the work is and why, then pass its id.\n"
                        "The handler validates kind/class and writes a "
                        "caused_by edge from this activity-intent's "
                        "context to cause_id. The literal 'autonomous' "
                        "was REMOVED 2026-05-30 (Adrian directive): it is "
                        "rejected -- there is no parentless action. The "
                        "earlier back-compat optional was retired because "
