...
def test_kg_add_auto_resolves_matching_enrichment(monkeypatch, config, palace_path, kg):
    ...
    # P2 cutover retired edge_traversal_feedback. The auto-accept signal
    # now lives purely in the kg_add triple write + the consumed
    # enrichment state; no feedback row is expected. The earlier
    # assertions (success, auto_resolved_enrichment present, pending
    # cleared) ARE the contract callers observe.
